...
async function getUserRecordings(accessToken) {
...
async function globalSearch(accessToken, search, type, date) {
...
function formatRecordingCard(recording, type) {
...
app.post('/mcp', async (req, res) => {
    ...
    try {
                    ...
                    case 'searchRecordings': {
                        ...
                        if (searchResults.recordings.length === 0 && searchResults.transcripts.length === 0) {
                            ...
                            if (searchResults.transcripts.length > 0) {
                                ...
                                searchResults.transcripts.forEach((transcript, index) => {
        ...
        return res.json(jsonRpcError(id, -32603, 'Internal error', error instanceof Error ? error.message : 'Unknown error'));
    }
});
